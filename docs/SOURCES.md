...
`config/sources.yml` — реестр того, откуда пайплайн *может* брать данные. Работает два
замка сразу:

1. **Реестр разрешает** — у источника нет `enabled: false` (владелец выключает так навсегда);
2. **Последняя реальная проверка прошла** — в `state/sources_health.json` у источника
   `status: ok`.

Итоговый вес источника — `categories[<category>].weight * source.weight`.
...
Скрипт `pipeline/verify_sources.py` проходит по всем `sources[]`, делает реальный запрос
и пишет вердикты в **`state/sources_health.json`**. Сам `sources.yml` он не трогает
(дамп YAML стёр бы все комментарии и дал дифф, который защита состояния отвергает).

- `status: ok` — HTTP 200 **и** (для `rss`/`atom`) `len(entries) > 0`, (для `json_api`)
  тело парсится как JSON и `items_path` разрешается в непустой список, (для `telegram`)
  превью отдаёт хотя бы один текстовый пост;
- `status: failed` — HTTP >= 400, таймаут, сетевая ошибка, фид пустой/не парсится,
  превью канала скрыто.

Еженедельно то же самое делает workflow `.github/workflows/verify-sources.yml`, так что
включение и выключение источников происходит без правки файлов руками. Блоки
`verification:` в самом YAML — заметки на момент составления реестра, не живой статус.
...
Одной страницы мало: активный канал пишет больше 20 постов между двумя сборами (07:00 и
16:00 по Дубаю), и новость, вышедшая утром, к вечеру уже уходила со страницы — так пропала
новость про бесплатное посуточное бронирование Dubizzle. Поэтому `collect.fetch_telegram`
листает страницы `?before=<message_id>` назад, пока посты не станут старше
`defaults.telegram_lookback_hours` (36 ч), но не больше `telegram_max_pages` страниц и
`telegram_max_items` постов на канал (config/sources.yml).
...
### 7.5 Приватные каналы

Ссылка-приглашение вида `t.me/+…` — это приватный канал. У него нет веб-превью, и бот его не
читает. Единственный путь — читать канал **от имени пользовательского аккаунта** (MTProto,
библиотека Telethon). Тип источника `telegram_private`, код — `pipeline/telegram_private.py`.

Как подключить — `docs/SETUP.md`, шаг «Приватные каналы». Кратко:
1. Аккаунт, от имени которого читаем, вступает в канал по ссылке-приглашению (в обычном
   приложении Telegram).
2. Один раз на своём компьютере запускается `scripts/telegram_login.py` — он печатает строку
   сессии и список каналов аккаунта с их `channel_id`.
3. Строка сессии кладётся в GitHub Secrets (`TELEGRAM_SESSION`), а `channel_id` — в
   `config/sources.yml` в запись `tg_private_1`.

**Ссылки-приглашения в репозиторий не записываются**: по ним может вступить любой, кто увидит
файл. В реестре канал адресуется только числовым `channel_id`, который сам по себе доступа не даёт.

**Безопасность.** Строка сессии — полноценный вход в аккаунт. Рекомендуется завести отдельный
технический аккаунт на другой сим-карте, подписать его на нужные приватные каналы и читать от его
имени; тогда даже утечка секрета не затронет личный аккаунт владельца.

**Что публикуем.** Постоянная ссылка на пост приватного канала (`t.me/c/…`) у читателей не
откроется, поэтому такие каналы работают в роли `signal`: тема берётся оттуда, а факты, цифры и
ссылка в нашем посте — из первоисточника.
